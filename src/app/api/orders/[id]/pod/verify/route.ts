...
export async function POST(
    ...
    try {
        ...
        if (!otp || otp.trim().length !== 6) {
            return NextResponse.json(
                { success: false, error: { code: 'VALIDATION_ERROR', message: 'Invalid verification code format. Code must be 6 digits.' } },
                { status: 400 }
            )
        }

        await ProofOfDeliveryService.verifyDeliveryOtp(id, otp.trim(), session.user.id)

        return NextResponse.json({
            success: true,
            message: 'Proof of Delivery verified and status updated successfully.',
        })
    } catch (err: any) {
        return NextResponse.json(
