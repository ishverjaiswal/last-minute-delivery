    ...
    switch (sortOrder) {
        ...
        case 'CUSTOMER_NAME':
            return sorted.sort((a, b) => {
                const nameA = a.customerName || `User #${a.customerId.slice(0, 8)}`
                const nameB = b.customerName || `User #${b.customerId.slice(0, 8)}`
                return nameA.localeCompare(nameB)
            })
        case 'AGENT_NAME':
            return sorted.sort((a, b) => {
                const nameA = a.agentName || (a.agentId ? `Agent #${a.agentId.slice(0, 8)}` : 'Unassigned')
                const nameB = b.agentName || (b.agentId ? `Agent #${b.agentId.slice(0, 8)}` : 'Unassigned')
                return nameA.localeCompare(nameB)
            })
        case 'ZONE':
            return sorted.sort((a, b) => {
...
export function highlightText(text: string | null | undefined, query: string): React.ReactNode {
    if (!text) return ''
